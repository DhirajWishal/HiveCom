...
struct Connection final {
  ...
  explicit Connection(const std::string &source,
...
template <class NodeType> struct NodeBuilder final {
  ...
  operator()(std::string_view identifier,
             const std::vector<std::string> &connections, NetworkGrid *pGrid) {
...
public:
  ...
  explicit NetworkGrid(const std::vector<Connection> &connections,
                       NodeBuilder<NodeType> builder) {
    // Setup the node grid.
    for (const auto &connection : connections)
      m_nodeMap[connection.m_source] =
          builder(connection.m_source, connection.m_connections, this);
  }
