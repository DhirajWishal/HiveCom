...
namespace HiveCom
{
    void AES256::encrypt(const Bytes &bytes, Byte padding /*= Byte()*/)
    {
        ...
        validate(EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr));

        // Set the IV size.
        validate(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, KeyAES256::IVSize, nullptr));

        // Initialize the key and initialization vector.
        validate(EVP_EncryptInit_ex(ctx, nullptr, nullptr, m_key.getKey().data(), m_key.getIV().data()));

        /// Provide the authentication data.
        if (m_key.getAuthentication().size() > 0)
        {
            validate(EVP_EncryptUpdate(ctx, nullptr, &len, m_key.getAuthentication().data(),
        ...
        for (const auto block : splitBytes(bytes, padding))
        {
            ...
            validate(EVP_EncryptUpdate(ctx, ciphertext.data(), &len, block.data(), block.size()));
            ciphertext_len = len;

            // Finalize the encryption.
            validate(EVP_EncryptFinal_ex(ctx, ciphertext.data() + len, &len));
            ciphertext_len += len;
        ...
        // Get the GCM tag.
        FixedBytes<16> tag;
        validate(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, tag.size(), tag.data()));
    ...
    Bytes AES256::decrypt(Byte padding /*= Byte()*/) const
    {
        ...
        validate(EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr));

        // Initialize the initializing vector.
        validate(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, KeyAES256::IVSize, nullptr));

        // Set the key and initialization vector.
        validate(EVP_DecryptInit_ex(ctx, nullptr, nullptr, m_key.getKey().data(), m_key.getIV().data()));

        // Provide the authentication data.
        if (m_key.getAuthentication().size())
        {
            validate(EVP_DecryptUpdate(ctx, nullptr, &len, m_key.getAuthentication().data(),
                                       m_key.getAuthentication().size()));
        }
        ...
        for (const auto block : splitBytes(m_cipherText, padding))
        {
            ...
            if (!EVP_DecryptUpdate(ctx, plaintextBlock.data(), &len, block.data(), block.size()))
                handleErrors();
        ...
        // Set the tag value.
        FixedBytes<16> tag;
        validate(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, tag.size(), tag.data()));

        // Finalize the decryption.
        ret = EVP_DecryptFinal_ex(ctx, plaintext.data() + len, &len);

        // Free the previously created context.
        EVP_CIPHER_CTX_free(ctx);
